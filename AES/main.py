...
def KeyExpansion(key, Nb, Nr, Nk):  # Nb分组大小 Nr轮数 Nk密钥长度
    w = hex2state(key, Nk)  # 此时结果示例：w = [[247, 218, 208, 56], [251, 61, 192, 171], [88, 187, 153, 135], [206, 74, 160, 243]]
    i = Nk
    while i < Nb * (Nr + 1):
        temp = w[i-1]
        if i % Nk == 0:
...
#     for i in range(4):
#         temp.append(int2list(strlist2hex(state[i])))
#     state = inMixColumns(temp)
#     state = AddRoundKey(state, keys, 10-x)
#     return state
...
def inRound(state, x):
    ...
    for i in range(4):
        temp.append(int2list(strlist2hex(state[i])))
    state = AddRoundKey(temp, keys, 10 - x)
    state = trans(state)
    state = inMixColumns(state)
    return state


def AES_128_encode(m, keys):
    ...
    for i in range(4):
        temp.append(int2list(strlist2hex(state[i])))
    state = AddRoundKey(temp, keys, 10)
    return state


def AES_128_decode(c, keys):
    state = AddRoundKey(c, keys, 10)
    for x in range(1, 10):
        state = inRound(state, x)
    # round 10
    state = inShiftRow(state)
    state = inByteSub(state)
    state = trans(state)
...
def multi_AES_128(T, s, keys, mode):
    if mode == '1':
        temp = AES_128_encode(s, keys)
        for i in range(T-1):
            temp = AES_128_encode(temp, keys)
    else:
        temp = AES_128_decode(s, keys)
        for i in range(T-1):
            temp = AES_128_decode(temp, keys)
    res = state2hex(temp)
    return res
