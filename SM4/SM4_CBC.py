...
def SM4_CBC(rk, m, strList, IV):
    if m == 1:  # 每次加密的输入是上次加密的输出和当前明文分组的异或
        s = en_padding(strList)
        l = len(s)
        c0 = '{:032x}'.format(int(IV, 16) ^ int(list2hexstr(s[0]), 16), 16)
        temp = SM4(c0, rk, 1)
        print(hexstr2list(temp))
        for i in range(1, l):
            ci = '{:032x}'.format(int(temp, 16) ^ int(list2hexstr(s[i]), 16), 16)
            temp = SM4(ci, rk, 1)
            print(hexstr2list(temp))
    else:
        t = 0
        ...
        l = len(s)
        temp0 = SM4(list2hexstr(s[0]), rk, 0)
        p0 = '{:032x}'.format(int(temp0, 16) ^ int(IV, 16), 16)
        print(hexstr2list(p0))
        for i in range(1, l-1):
            tempi = SM4(list2hexstr(s[i]), rk, 0)
            pi = '{:032x}'.format(int(tempi, 16) ^ int(list2hexstr(s[i-1]), 16), 16)
            print(hexstr2list(pi))
        temp_last = SM4(list2hexstr(s[l-1]), rk, 0)
        p_last = '{:032x}'.format(int(temp_last, 16) ^ int(list2hexstr(s[l-2]), 16), 16)
        b = int(p_last[30:32], 16)  # 解码要把填充的数据删掉.注意填充的数据恢复成明文才能看到
        for j in range(b):
            if p_last[30-2*j:32-2*j] == p_last[30:32]:
                t += 1
        if t == b:
            for i in range(16-b):
                print('0x'+p_last[2*i:2*i+2], end=' ')
