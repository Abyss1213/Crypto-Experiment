...
def fenzu(strList, n):  # 将strList按n个字节为一组分组
    p = []
    N = len(strList) // n
    m = len(strList) % n
    if m == 0:
        for i in range(N):
            pi = '0x'
            for j in range(n):
                pi = pi + '{:02x}'.format(int(strList[n * i + j], 16), 16)
            p.append(int(pi, 16))
    else:
        N += 1
        for i in range(N-1):
        ...
        p_last = '0x'
        for j in range(m):
            p_last = p_last + '{:02x}'.format(int(strList[n * (N-1) + j], 16), 16)
        p.append(int(p_last, 16))
    return p


def SM4_CFB(n, rk, p, IV, M):
    N = len(strList) // n
    mod = len(strList) % n
    if mod != 0:
        N += 1
    temp0 = SM4(IV, rk, 1)
    s0 = int(temp0[0: 2*n], 16)  # 注意这里两位十六进制数才一字节
    c0 = hex(p[0] ^ s0)[2:].zfill(2*n)  # '0x7800'.strip('0x') == '78'，会把后面的00丢掉
    print(hexstr2list(c0, n))
    xi = IV
    ci = c0
    for i in range(1, N):
        if M == 1:
            ...
            xi = xi[2 * n:] + hex(p[i-1])[2:].zfill(2*n)
        tempi = SM4(xi, rk, 1)
        if i < N-1:
            si = int(tempi[0: 2 * n], 16)
            ci = hex(p[i] ^ si)[2:].zfill(2*n)
            print(hexstr2list(ci, n))
        else:
            if mod == 0:
                si = int(tempi[0: 2 * n], 16)
                ci = hex(p[i] ^ si)[2:].zfill(2*n)
                ...
                si = int(tempi[0: 2 * mod], 16)
                ci = hex(p[i] ^ si)[2:].zfill(2*mod)
                print(hexstr2list(ci, mod))


n = int(input())
k = input().strip().replace('\n', '').replace('\r', '')
IV = input().strip().replace('\n', '').replace('\r', '')
M = int(input())  # 有用，虽然CFB只用加密算法，但是加解密结构不一样
k = '{:032x}'.format(int(k, 16), 16)
IV = '{:032x}'.format(int(IV, 16), 16)
rk = KeyExpansion(k)
strList = []
