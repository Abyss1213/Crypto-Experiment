...
def padding(strList):  # PKCS5 CTR不要填充，这就是为了后续类型转换使用
    s = []
    l = len(strList)
    k = l // 16
    m = l % 16
    ...
    if m != 0:  # 如果输入数量不是16的整数倍
        ...
        for i in range(p):
            s[k].append('0x00')  # 给最后一组补上0，异或方便一点
    else:
        for j in range(16):  # 还是要注意考虑一下最后一组
            s[k].append(strList[16 * (k-1) + j])
    return s


def SM4_CTR(rk, strList, IV):
    # 生成计数器
    N = len(strList) // 16
    m = len(strList) % 16
    if m != 0:
    ...
    counter = []
    for i in range(N):
        counteri = '{:032x}'.format(counter0 + i, 16)
        counter.append(counteri)
    # 生成密/明文流
    s = padding(strList)
    for i in range(N):
        tempi = int(SM4(counter[i], rk, 1), 16)
        pi = int(list2hexstr(s[i]), 16)
        ci = '{:032x}'.format(tempi ^ pi, 16)
        if i < N-1:
            print(hexstr2list(ci))
        else:
            if m != 0:
                for j in range(m):
                    print('0x'+ci[2*j:2*j+2], end=' ')
            else:
                ci = '{:032x}'.format(tempi ^ pi, 16)
                print(hexstr2list(ci))
