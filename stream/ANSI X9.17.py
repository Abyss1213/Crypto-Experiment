...
def DES3(s, keys1, keys2, m):
    if m == 1:
        temp = DES(s, keys1, 1)
        temp = DES(temp, keys2, 0)
        res = DES(temp, keys1, 1)
    else:
        temp = DES(s, keys1, 0)
        temp = DES(temp, keys2, 1)
        res = DES(temp, keys1, 0)
    return hexy(res)


IV = input().strip().replace('\n', '').replace('\r', '')
key1 = input().strip().replace('\n', '').replace('\r', '')
key2 = input().strip().replace('\n', '').replace('\r', '')
n = int(input())

T = []
for i in range(n):
    t = input().strip().replace('\n', '').replace('\r', '')
    t_bin = int(bin(int(t, 16)).replace('0b', ''), 2)
    T.append('{:064b}'.format(t_bin))

IV_bin = int(IV, 16)
key1_bin = '{:064b}'.format(int(key1, 16))
key2_bin = '{:064b}'.format(int(key2, 16))

key1_bin = PC1(key1_bin)  # PC1只要最开始用一次
keys1 = get_keys(key1_bin)
key2_bin = PC1(key2_bin)  # PC1只要最开始用一次
keys2 = get_keys(key2_bin)

V = IV_bin
for i in range(n):
    XR = '{:064b}'.format(V ^ DES3(T[i], keys1, keys2, 1))
    R = DES3(XR, keys1, keys2, 1)
    XV = '{:064b}'.format(R ^ DES3(T[i], keys1, keys2, 1))
    V = DES3(XV, keys1, keys2, 1)
    print('0x' + '{:016x}'.format(R, 16))
