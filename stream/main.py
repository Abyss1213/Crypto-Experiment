...
def DES3(s, key1, key2, m):
    if m == 1:
        temp = DES(s, key1, 1)
        temp = DES(temp, key2, 0)
        res = DES(temp, key1, 1)
    else:
        temp = DES(s, key1, 0)
        temp = DES(temp, key2, 1)
        res = DES(temp, key1, 0)
    return hexy(res)
...
V = IV_bin
for i in range(n):
    XR = '{:064b}'.format(V ^ DES3(T[i], key1_bin, key2_bin, 1))
    R = DES3(XR, key1_bin, key2_bin, 1)
    XV = '{:064b}'.format(R ^ DES3(T[i], key1_bin, key2_bin, 1))
    V = DES3(XV, key1_bin, key2_bin, 1)
    if i % 6 == 0:
        print('0x' + '{:016x}'.format(R, 16), end='\n')
    else:
        print('0x' + '{:016x}'.format(R, 16), end=' ')
